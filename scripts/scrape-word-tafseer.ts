...
async function splitWordByKey(
  ...
  for (const str of splitList) {
    ...
    insertWords.push([
      `${suraId}:${verseId}:${possId + increaseIdxBy}`,
      newObj,
    ]);
    increaseIdxBy++;
  }
...
async function mergeWordMap(key: string, aggregated: WordMap) {
  ...
  insertWords.push([
    `${suraId}:${verseId}:${possId++}`,
    <WordObj>removedWords[0],
  ]);
...
async function testItWithPages(aggregated: WordMap) {
  const pages = JSON.parse(
    readFileSync("../src/data/pages/hafs-unicode/pages.json", "utf8"),
  );
  let poss = 1;
      ...
      for (let k = 0; k < words.length; k++) {
        ...
        if (words[k]["charType"] === "word") {
          ...
          if (aggregatedText != pagesText) {
            console.log(
              `Warning: not same words ${pagesText} , ${aggregatedText} with key ${key}`,
            );
          }
        } else {
  ...
  console.log(
    "please check  all warning and fix them with merge and split lists",
  );
...
async function main() {
  ...
  const outputPaths = [
    "playground/vite-react/data/tafseers/quran-words.json",
    "playground/vite-typescript/data/tafseers/quran-words.json",
  ];

  for (const outputPath of outputPaths) {
    const dir = outputPath.substring(0, outputPath.lastIndexOf("/"));
    mkdirSync(dir, { recursive: true });
    writeFileSync(outputPath, JSON.stringify(sortedAggregated, null, 2));
